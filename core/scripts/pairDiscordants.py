...
def pairReads(inFile, numMappings, order, dist, minSpan, minConcRange, maxConcRange, mode, anchorThresh, multiThresh, editSlop):
	...
	while 1:
		line = data.readline()
		if not line:
			# Original code; modified below as per 
			# http://code.google.com/p/hydra-sv/wiki/TypicalWorkflow  
			#
			...
			if (mappings1 > 0) and (mappings2 > 0):
				pairs = makePairs(read1, read2, numMappings, order, dist, minSpan, minConcRange, maxConcRange, mode, mappings1, mappings2)
				if pairs != "BAD":
						editDistance = pairs[0][15] # Original: editDistance = pairs0?15?
						if mode == "hydra":
							printHydraMappings(pairs, editDistance, editSlop)
						elif mode == "discordant":
							printDiscordantMappings(pairs, editDistance, editSlop)
			...
			# http://code.google.com/p/hydra-sv/wiki/TypicalWorkflow has this 
			# break indented one place left. Since this is always end the 
			# loop it is probably incorrect, so I'm moving it to the right.
