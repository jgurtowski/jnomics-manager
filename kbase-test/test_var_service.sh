...
jkbase fs -put_pe yeastrename.1.fq yeastrename.2.fq yeastrename_test

jmessage "Aligning reads"
#run alignment
align_jobid=`jkbase compute bwa -in yeastrename_test.pe -out yeastrename_test_bwa -organism yeastrename | cut -d ':' -f 2`
...
#run snp
snp_jobid=`jkbase compute snp -in yeastrename_test_bwa -out yeastrename_test_snp -organism yeastrename | cut -d ':' -f 2`
...
#merge vcf
jkbase compute vcf_merge -alignments yeastrename_test_bwa -in yeastrename_test_snp -out yeastrename_test.vcf

jmessage "Downloading complete vcf"
#download vcf
jkbase fs -get yeastrename_test.vcf
