 ...
 * User: james
 */
public class ShockBatchWrite extends FSBase{
    ...
    public void handle(List<String> remainingArgs, Properties properties) throws Exception {
        ...
        if(help || hdfs_files == null || hdfs_Output == null || hdfs_path == null){
        	System.out.println("shock_batch_write -hdfs_files=<Name1,Name2,Name3,Name4> -hdfs_Output=<hdfs Output> -hdfs_path=<Input Path>");
        }else{
        	List<String> items = Arrays.asList(hdfs_files.split("\\s*,\\s*"));
        	for(String item : items){
        	System.out.println(item);
        	}
        	JnomicsThriftJobID jobID = client.ShockBatchWrite(items, hdfs_Output,hdfs_path, auth);
